...
export default function() {
  // Configuration
  const catalogApiUrl = 'http://localhost:5133/api/v1/catalog';
  
  // Make request to get catalog items
  const itemsResponse = http.get(`${catalogApiUrl}/items?pageSize=9&pageIndex=0`);
  ...
  if (itemsSuccess && itemsResponse.json().data.length > 0) {
    ...
    const itemDetailResponse = http.get(`${catalogApiUrl}/items/${itemId}`);
    ...
    // Get catalog brands
    const brandsResponse = http.get(`${catalogApiUrl}/catalogbrands`);
    ...
    // Get catalog types
    const typesResponse = http.get(`${catalogApiUrl}/catalogtypes`);
...
export function setup() {
  const catalogApiUrl = 'http://localhost:5133/api/v1/catalog';
  const res = http.get(`${catalogApiUrl}/items?pageSize=1&pageIndex=0`);
